...
5. 開発者として、Vision (LLaVA等, Ollama経由) で画像の内容を理解した上で検索結果に含められる
...
5. 画像アップロード時に Vision (LLaVA等, Ollama経由) で内容説明テキストが生成され、テキスト埋め込みとしても検索可能になること
...
- Vision (LLaVA等, Ollama経由) による画像内容テキスト生成の統合
...
- **Vision推論の負荷**: ローカルVision (LLaVA等) は画像あたりの推論時間/メモリ負荷が大きい。バッチ処理・GPU活用で制御する
